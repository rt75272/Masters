...
def get_user_input():
    """Prompt the user for input with a timeout. If the user does not respond within
    30 seconds, ask if they are still there. If there is still no response, exit.
    """
    try:
        return inputimeout(prompt="You: ", timeout=30)
...
def get_chatbot_response(client, conversation):
    """Get the chatbot's response from OpenAI and return the reply, given the conversation history.
    """
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
