...
#
# Generates a billion random integers and saves them to a file named hugefile1.txt. Also, 
...
multithread() {
    ...
    for i in $(seq 1 $num_cores); do
        if [ $remaining -gt 0 ]; then
            # Calculate how many numbers each core should generate.
            count=$((remaining / num_cores)) # Each core gets an equal number.
            if [ $i -eq $num_cores ]; then
                count=$remaining # Last core gets the remaining numbers.
            fi
            looper $i $count & # Run looper for the current core in the background.
...
main() {
    ...
    multithread # Run several threads to produce the billion random integers faster.
